...
void Math::InterpolateLine(int* x, int* y, int end_x, int end_y, int step_size)
{
	...
	if (step_size <= 0 || (*x == end_x && *y == end_y)) {
		return;
	}
...
/*
	...
	if agrees, differing at most by 1 from the old result, and
	in all such cases I checked, this one was the more accurate
	result (this version rounds rather than truncates).
	It was also tested for all x,y from -1000 to 1000 versus atan2 funcion
	and never differed more than 2 from "true" result (which often
	...
	Most importantly, it gives correct results on inputs where the old version
	crashes (e.g. -1, 376), which prompted Russ to rewrite this in clear C
	to begin with.
*/
...
//
bool Math::PointInPoly(int const xt, int const yt, int const * const vertex, int const nvertex) 
{
	int xnew, ynew;
	int xold, yold;
	int x1, y1;
...
RECT Math::BoundingBox(const POINT& pt1, const POINT& pt2, const POINT& pt3, const POINT& pt4)
{
	RECT bounding;
...
RECT Math::BoundingBox(const POINT* verts, const unsigned int nverts) 
{
	//Nv_ASSERT(nverts > 0);
...
{
	float result = Wrap2Pi(wrapMe + Nv_PI);
	return (result - Nv_PI);
}
...
{
	if (wrapMe > Nv_2PI) {
		wrapMe = fmod(wrapMe, Nv_2PI);
	}
	else if (wrapMe < 0) {
		wrapMe = Nv_2PI - fmod(fabs(wrapMe), Nv_2PI);
	}
