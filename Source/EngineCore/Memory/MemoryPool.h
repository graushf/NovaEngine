...
// to the next chunk, making the poool a singly-linked list of memory chunks.
//
...
// reinitialize the entire pool. The chunk size is the sieze of each chunk, minus the header, in
// bytes. The memory pool will allocate the appropriate amount of memory and set up the data
// structure in the Init() call. Thus, total memory usage will be N * (S + 4) + 0, where N is the
...
// choose your intial sizes carefully.
//
