// ================================================================================
// HumanView.h : Implements the class HumanView, which provides a Human interface
// into the game
//
// ================================================================================
...
HumanView::HumanView(std::shared_ptr<IRenderer> renderer)
{
	...
	if (renderer)
	{
		...
		m_pScene.reset(Nv_NEW ScreenElementScene(renderer));

		Frustum frustum;
		frustum.Init(Nv_PI / 4.0f, 1.0f, 1.0f, 100.0f);
		//m_pCamera.reset(Nv_NEW CameraNode(&Mat4x4::g_Identity, frustum));
		//Nv_ASSERT(m_pScene && m_pCamera && _T("Out of memory"));
...
void HumanView::VOnRender(double fTime, float fElapsedTime)
{
	...
	if (m_runFullSpeed || ((m_currTick - m_lastDraw) > SCREEN_REFRESH_RATE))
	{
		if (g_pApp->m_Renderer->VPreRender())
		{
			...
			// record the last succesful paint
...
bool HumanView::InitAudio()
{
	...
	if (!g_pAudio)
	{
		g_pAudio = Nv_NEW DirectSoundAudio();
	}
...
//
// Aggregates calls to manage event listeners for th HumanView class
//
...
/*
void HumanView::PlaySoundDelegate(IEvenDataPtr pEventData)
{
	//std::shared_ptr<EvtData_PlaySound> pCastEventData = static_pointer_cast<EvtData_PlaySound>(pEventData);

	// play the sound a bullet makes when it hits a teapot
	//Resource resource(pCastEventData->GetResource().c_str());
	//std::shared_ptr<ResHandle> srh = static_pointer_cast<ResHandle>(g_pApp->m_ResCache->GetHandle(&resource));
	//std::shared_ptr<SoundProcess> sfx(Nv_NEW SoundProcess(srh, 100, false));
...
// ----------------------------------------------------------------------------
// HumanView::Console - defines the class to manage a console to type in commands
//
