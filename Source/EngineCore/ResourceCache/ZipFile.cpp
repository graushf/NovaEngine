//==========================================================================================
// ZipFile.cpp : The API to use Zip files
//==========================================================================================
...
bool ZipFile::Init(const std::wstring &resFileName)
{
	...
	m_pDirData = Nv_NEW char[dh.dirSize + dh.nDirEntries * sizeof(*m_papDir)];
	if (!m_pDirData)
		return false;
...
bool ZipFile::ReadFile(int i, void *pBuf)
{
	...
	char *pcData = Nv_NEW char[h.cSize];
	if (!pcData)
		return false;
...
bool ZipFile::ReadLargeFile(int i, void *pBuf, void(*progressCallback)(int, bool &))
{
	...
	if (err == Z_OK)
	{
		...
		while (stream.total_in < (uInt)h.cSize && !cancel)
		{
			...
			{
				//Nv_ASSERT(0 && "Something happened.");
				break;
			}
