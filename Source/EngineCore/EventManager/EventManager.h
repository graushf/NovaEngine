...
//
// The interface to this construct uses smart pointer wrapped objects, the purpose
// being to ensure that no object that the registry is referring to is destroyed 
// before it is removed from the registry AND to allow for the registry to be
// the only place where this list is kept... the application code does not need to
// maintain a second list.
//
// Simply tearing down the registry (e.g.: destroying it) will automatically clean up
// all pointed-to objects (so long as there are no other ourstanding references, of
// course.)
//
// -----------------------------------------------------------------------------------------------
class IEventManager
{
public:
	enum eContents { kINFINITE = 0xffffffff };

	explicit IEventManager(const char* pName, bool setAsGlobal);
	virtual ~IEventManager(void);

	// Registers a delegate function that will get called when the event type is triggered. Returns
	...
	// Find the next-available instance of the named event type and remove it from the processing queue. This may
	// be done up to the point that it is actively being processed... e.g.: is safe to happen during event
	// processing itself.
	//
	...
	// Is is not valid to have more than one global event manager.
