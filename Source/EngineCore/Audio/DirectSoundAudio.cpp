//========================================================================
// DirectSoundAudio.cpp : Defines a simple sound system that uses
//							DirectSound.
//========================================================================
...
//
IAudioBuffer* DirectSoundAudio::VInitAudioBuffer(std::shared_ptr<ResHandle> resHandle) // const
{
	std::shared_ptr<SoundResourceExtraData> extra = static_pointer_cast<SoundResourceExtraData>(resHandle->GetExtra());
	...
	switch (extra->GetSoundType())
	{
		...
		case SOUND_TYPE_MIDI:	// If it's a midi file, then do nothin at this time... maybe we will support this in the future
			//Nv_ASSERT(false && "MP3s and MIDI are not supported");
			return NULL;
			break;

		default:
			//Nv_ASSERT(false && "Unknown sound type");
			return NULL;
	} // end switch
	...
	// since each requires some overhead and limits if the buffer can be
	...
	IAudioBuffer* audioBuffer = Nv_NEW DirectSoundAudioBuffer(sampleHandle, resHandle);
...
DirectSoundAudioBuffer::DirectSoundAudioBuffer(
	...
	std::shared_ptr<ResHandle> resource)
...
void DirectSoundAudioBuffer::VSetVolume(int volume)
{
	...
	// range to avoid an annoying silence for the lower 50% of a volume slider that uses a logarithmic
	// scale. 
	// This was contributed by BystanderKain!
	int nvDSBVolumeMin = DSBVOLUME_MIN;
	...
	// converts volume from 0-100 into range for DirectX - don't forget to use logarithmic scale!
	float coeff = (float)volume / 100.0f;
	float logarithmicProportion = coeff > 0.1f ? 1 + log10(coeff) : 0;
	float range = float(DSBVOLUME_MAX - nvDSBVolumeMin);
	float fvolume = (range * logarithmicProportion) + nvDSBVolumeMin;

	//Nv_ASSERT(fvolume >= nvDSBVolumeMin && fvolume <= DSBVOLUME_MAX);
...
//
//		Restores the lost buffer. *pbWasRestored returns TRUE if the buffer was restored.
//		It can also NULL if the information is not needed.
//
HRESULT DirectSoundAudioBuffer::RestoreBuffer(BOOL* pbWasRestored)
{
	...
	if (dwStatus & DSBSTATUS_BUFFERLOST)
	{
		...
		// If it does, sleep until DirectSound gives us control but
		// fail if it goes on more than 1 second.
		int count = 0;
		do
		{
			hr = m_Sample->Restore();
...
HRESULT DirectSoundAudioBuffer::FillBufferWithSound(void)
{
	...
	// an app which ahd a DirectSound device
	if (FAILED(hr = RestoreBuffer(NULL))) {
		return DXUT_ERR(L"RestoreBuffer", hr);
	}

	int pcmBufferSize = m_Resource->Size();
	std::shared_ptr<SoundResourceExtraData> extra = static_pointer_cast<SoundResourceExtraData>(m_Resource->GetExtra());

	// Lock the buffer down
	if (FAILED(hr = m_Sample->Lock(0, pcmBufferSize, &pDSLockedBuffer, &dwDSLockedBufferSize,
		NULL, NULL, 0L))) {
