...
MovementController::MovementController(std::shared_ptr<SceneNode> object, float initialYaw, float initialPitch, bool rotateWhenLButtonDown)
	: m_object(object)
{
...
void MovementController::OnUpdate(DWORD const deltaMilliseconds)
{
	...
	if (m_bKey['Q'])
	{
		...
		// spot so I can take some screen shots
		...
		D3DXMatrixMultiply(&m_matFromWorld, nullptr, &m_matToWorld);
	...
	*/
	...
	// Handling rotations as a result of mouse position
	{
		// Secret formula
		...
		// Creates the new object-to-world matrix, and the
