...
// ================================================================
// SafeStream.cpp : Defines a template for thread safe streams
// ================================================================
...
protected:
	...
	virtual int sync()
	{
		m_pOwnerStream->on_new_message(ger_stream_buffer().str());
	...
	virtual streambuf_type* setbuf(char_type* buffer, std::streamsize n)
	{
		...
		//		we don't use a buffer - we forward
		//		everything
		//Nv_ASSERT(buffer == NULL && n == 0);
		this->setp(NULL, NULL);
		return this;
	}
...
DWORD WINAPI WriteToLog(LPVOID lpData)
{
	...
	for (int idx = 0; idx < WRITES_PER_THREAD; idx++)
	{
		...
		{
			// from now on, '5.23' will be writen as '5,23'
			// (german locale)
...
void testStreaming(CProcessManager* procMgr)
{
	...
	while (true)
	{
		InterlockedIncrement(&nRemainigThreads);
